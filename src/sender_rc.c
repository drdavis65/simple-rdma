...
static int modify_qp_to_rtr(struct sender_context *ctx,
...
int main(int argc, char *argv[])
{
	...
	if (posix_memalign((void **)&send_ctx->buf, sysconf(_SC_PAGESIZE),
			   send_ctx->size)) {
	...
	send_ctx->mr = ibv_reg_mr(send_ctx->pd, send_ctx->buf, send_ctx->size,
	...
	send_ctx->cq = ibv_create_cq(send_ctx->ctx, send_ctx->num_packets, NULL,
				     send_ctx->channel, 0);
	if (!send_ctx->cq) {
	...
	}
	{
		...
		if (ibv_modify_qp(send_ctx->qp, &attr,
	...
	struct ibv_ah_attr ah_attr = {
		.is_global = 1,
		.port_num = 1,
		.grh = { .dgid = remote_info
					 .gid, // Use remote GID from connection exchange
			 .flow_label = 0,
	...
	if (send_ctx->qpx) {
		...
		ibv_wr_rdma_write_imm(
			send_ctx->qpx, send_ctx->remote_rkey,
